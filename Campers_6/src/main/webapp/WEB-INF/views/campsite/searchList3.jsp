...
<c:set var = "thisPage" value="${window.location.href }"/>
...
<div class="container-fluid">
	...
	<div class="row">
		...
		<c:if test="${searchSite eq '일반야영장'}">
			<h2 class="mb-4">캠핑 - 캠핑장 검색</h2>
			<input type="hidden" class="class" name="searchSite" id="searchSite" type = "text" value="일반야영장">
		</c:if>
		<c:if test="${searchSite eq '자동차야영장' }">
			<h2 class="mb-4">오토캠핑 - 캠핑장 검색</h2>
			<input type="hidden" class="class" name="searchSite" id="searchSite" type = "text" value="자동차야영장">
		</c:if>
		<c:if test="${searchSite eq '글램핑' }">
			<h2 class="mb-4">글램핑 - 캠핑장 검색</h2>
			<input type="hidden" class="class" name="searchSite" id="searchSite" type = "text" value="글램핑">
		</c:if>
		<c:if test="${searchSite eq '카라반' }">
			<h2 class="mb-4">카라반 - 캠핑장 검색</h2>
			<input type="hidden" class="class" name="searchSite" id="searchSite" type = "text" value="카라반">
		</c:if>
			<hr class="my-4">
				<div class="row">
							...
							<div class="col-sm-6 mb-5 hover-animate"
								...
								<div class="card h-100 border-0 shadow">
									<div class="card-img-top overflow-hidden gradient-overlay">
										...
										<a class="tile-link" href="${path }/campsite/view?campsite_index=${campsite.campsite_index}">
										<input type="hidden" /></a>
			...
			<nav aria-label="Page navigation example">
				<ul
					...
					<li class="page-item"><a class="page-link"
						href="${url}&page=${ pageInfo.prvePage }">
					...
					<c:forEach begin="${ pageInfo.startPage }"
						...
						<c:if test="${ pageInfo.currentPage != status.current}">
							<li class="page-item "><a class="page-link "
								href="${ requestUrl }&page=${ status.current }"><c:out
										value="${ status.current }" /></a></li>
					...
					<li class="page-item "><a class="page-link "
						href="${ URL }&page=${ pageInfo.nextPage }">
		...
		<div class="container">
			<div class="row">
				<div class="col-lg-4 mb-5 mb-lg-0">
					<div class="mb-3">
						<img src="${path}/resources/img/newphoto/camper_logo_text_removebg.png"
							alt="Directory logo">
					</div>
				</div>
